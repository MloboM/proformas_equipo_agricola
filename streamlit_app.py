...
def save_uploaded_image(file, prefix: str = "img") -> str:
    ...
    if ext not in ['.jpg', '.jpeg', '.png', '.gif']:
        st.error("Formato de imagen no válido. Use JPG, PNG o GIF.")
        return ""
...
with st.sidebar:
    ...
    st.metric("Clientes", stats["active_customers"])
    st.metric("Modelos", stats["total_models"])
    st.metric("Proformas", stats["total_proformas"])
    
    st.markdown("---")
    st.caption("🚜 AgriQuote v2.0")
    st.caption("© 2025 Colono")


# ========================= PÁGINA: INICIO =========================

if menu_option == "🏠 Inicio":
    st.markdown('<p class="main-header">🚜 Bienvenido a AgriQuote</p>', unsafe_allow_html=True)
    
    st.markdown("""
    ### Sistema de Gestión de Cotizaciones Agrícolas
    
    **AgriQuote** es tu herramienta completa para:
    - 📝 Crear cotizaciones profesionales de tractores e implementos
    - 👥 Gestionar clientes y asesores
    - 🏭 Mantener catálogo de marcas y modelos
    - ⚙️ Personalizar términos, logos e IVA
    - 📄 Generar PDFs con paginación automática
    - 📊 Ver y filtrar todas las cotizaciones
    """)
    ...
    st.markdown("""
    1. **Configurar datos básicos:** Ve a Mantenimientos para registrar clientes, asesores, marcas y modelos
    2. **Personalizar configuración:** Ajusta logos, términos, condiciones y datos de la empresa
    3. **Crear cotización:** Selecciona productos, ajusta IVA si es necesario y genera tu primera proforma en PDF
...
# ========================= VER PROFORMAS =========================
    ...
    with st.expander("🔍 Filtros de búsqueda", expanded=True):
        ...
        with col1:
            # Filtro por cliente
            with SessionLocal() as db:
                customers = crud.list_customers(db, active_only=False)
            customer_filter_options = ["Todos"] + [f"[{c.id}] {c.name}" for c in customers]
            customer_filter = st.selectbox("Cliente", customer_filter_options)
        
        with col2:
            # Filtro por asesor
            with SessionLocal() as db:
                advisors = crud.list_advisors(db, active_only=False)
            advisor_filter_options = ["Todos"] + [f"[{a.id}] {a.name}" for a in advisors]
            advisor_filter = st.selectbox("Asesor", advisor_filter_options)
    ...
    # Obtener proformas con filtros
    with SessionLocal() as db:
        # Aplicar filtros
        customer_id_filter = None
        if customer_filter != "Todos":
            customer_id_filter = int(customer_filter.split("]")[0].replace("[", ""))
        
        # Obtener todas las proformas
        all_proformas = crud.list_proformas(db, limit=1000, customer_id=customer_id_filter)
        
        # Filtrar por asesor
        if advisor_filter != "Todos":
            advisor_id_filter = int(advisor_filter.split("]")[0].replace("[", ""))
            all_proformas = [p for p in all_proformas if p.advisor_id == advisor_id_filter]
        
        # Filtrar por fechas
        all_proformas = [
            p for p in all_proformas
            if date_from <= p.date.date() <= date_to
        ]
        
        # Construir datos para mostrar
        proformas_data = []
        for p in all_proformas:
            # Obtener nombres mientras la sesión está activa
    ...
    if proformas_data:
        st.markdown(f"### 📋 Se encontraron {len(proformas_data)} proformas")
        ...
        # Si hay una fila seleccionada, mostrar opciones
        if df_display.selection.rows:
            ...
            with col2:
                # Duplicar proforma
                if st.button("📋 Duplicar Proforma", use_container_width=True):
                    st.session_state.duplicate_proforma_id = selected_proforma['id']
                    st.session_state.show_duplicate_form = True
                    st.rerun()
            
            with col3:
                # Eliminar proforma
                if st.button("🗑️ Eliminar Proforma", use_container_width=True, type="secondary"):
                    if st.checkbox("Confirmar eliminación"):
                        try:
                            with SessionLocal() as db:
                                crud.delete_proforma(db, selected_proforma['id'])
                            st.success("✅ Proforma eliminada")
                            st.rerun()
                        except Exception as e:
                            st.error(f"❌ Error: {e}")
    else:
        st.info("No se encontraron proformas con los filtros seleccionados.")
    
    # Formulario de duplicación (si está activo)
    if st.session_state.get("show_duplicate_form", False):
        st.markdown("---")
        st.markdown("### 📋 Duplicar Proforma")
        
        with SessionLocal() as db:
            original = crud.get_proforma(db, st.session_state.duplicate_proforma_id)
            
            if original:
                with st.form("duplicate_form"):
                    st.info(f"Duplicando proforma: {original.number}")
                    ...
                    st.markdown("💡 La nueva proforma mantendrá los mismos productos, cliente y configuración.")
                    
                    col1, col2 = st.columns(2)
                    
                    with col1:
                        if st.form_submit_button("✅ Crear Duplicado", use_container_width=True):
                            try:
                                ...
                                for item in original.items:
                                    items_data.append({
                                        ...
                                        "tax_rate": getattr(item, 'tax_rate', 13)  # Mantener IVA original
                                    })
                                
                                # Crear nueva proforma
                                new_proforma = crud.create_proforma(
                                    db,
                                    number=new_number,
                                ...
                                st.success(f"✅ Proforma duplicada: {new_number}")
                                st.session_state.show_duplicate_form = False
                                st.rerun()
                            except Exception as e:
                                st.error(f"❌ Error: {e}")
...
# ========================= MANTENIMIENTO: MARCAS =========================
    ...
    with tab1:
        ...
        if brands_data:
            st.dataframe(brands_data, use_container_width=True, hide_index=True)
...
# ========================= CONFIGURACIÓN =========================
    ...
    with tabs[1]:
        ...
        with col1:
            ...
            if st.button("💾 Actualizar Logo Izquierdo", use_container_width=True):
                if logo_left_file:
        ...
        with col2:
            ...
            if st.button("💾 Actualizar Logo Derecho", use_container_width=True):
                if logo_right_file:
    ...
    with tabs[2]:
        ...
        with st.form("terms_tractor_form"):
            ...
            with col2:
                if st.form_submit_button("📄 Restablecer", use_container_width=True):
...
# ========================= NUEVA PROFORMA =========================
                ...
                with st.expander(f"🔧 {model_info['brand_name']} - {model_info['model_name']}", expanded=True):
                    ...
                    with cols[3]:
                        unit_price = st.number_input(
                            "Precio Unit.",
                            min_value=0.0,
                            value=float(model_info['base_price']),
                            step=100.0,
                            format="%.2f",
    ...
    if st.session_state.pdf_generated and st.session_state.pdf_path:
        ...
        if st.button("🆕 Crear Nueva Proforma", use_container_width=True):
            st.session_state.pdf_generated = False
            st.session_state.pdf_path = None
            st.session_state.pdf_info = {}
            st.rerun()
